...
class FileUploadComponent extends Object{
  ...
  /*************************************************
    ...
    *
    * Nick Milosevic Mods: remove resized images created by the FileUpload Helper as well.
    * @author Nick Milosevic
  ...
  function removeFile($name = null){
    ...
    //break the file name up so we can use it in a regular expression
    preg_match('/^(.*)\.(\w{3})$/', $name, $file);
     
    //get all the images from $up_dir
    $files = scandir($up_dir);
     
     /************* 
      * filter out all the images that we aren't concerned with. ie: only get the resized images that are associated with $name
      * 
      * $file[0]: full filename and extension... (the same as $name)
      * $file[1]: just the filename
      * $file[2]: just the extension
      */
    $files = preg_grep('/^' . $file[1] . 'x\d+\.' . $file[2] . '$/', $files);
 
    //delete each of the resized images associated with $name
    foreach ($files as $file) {
     	if (!unlink($up_dir . DS . $file)){
        return false;
      }
    }
